...
class DatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    companion object {
        const val DATABASE_NAME = "VideoJuegoAppV2.db"
        ...
        const val COLUMN_VIDEOJUEGO_LATITUDE = "latitude"
        const val COLUMN_VIDEOJUEGO_LONGITUDE = "longitude"
    ...
    override fun onCreate(db: SQLiteDatabase) {
        ...
        db.execSQL(
            "CREATE TABLE $TABLE_VIDEOJUEGO (" +
                    ...
                    "$COLUMN_VIDEOJUEGO_LATITUDE REAL, " +
                    "$COLUMN_VIDEOJUEGO_LONGITUDE REAL)"
        )
...
//        if (oldVersion < 2) {
//            val alterTableQuery = "ALTER TABLE video_juegos ADD COLUMN latitude REAL"
//            db.execSQL(alterTableQuery)
//            val alterTableQuery2 = "ALTER TABLE video_juegos ADD COLUMN longitude REAL"
...
//        }
    ...
    fun addVideoJuego(videoJuego: VideoGame): Long {
        ...
        val values = ContentValues().apply {
            ...
            put(COLUMN_VIDEOJUEGO_FECHA_LANZAMIENTO, videoJuego.fechaLanzamiento)
            put(COLUMN_VIDEOJUEGO_DISPONIBLE, if (videoJuego.disponible) 1 else 0)
            put(COLUMN_VIDEOJUEGO_LATITUDE, videoJuego.latitude)
            put(COLUMN_VIDEOJUEGO_LONGITUDE, videoJuego.longitude)
        }
    ...
    fun getAllVideoJuegos(): List<VideoGame> {
        ...
        while (cursor.moveToNext()) {
            val videoJuego = VideoGame(
                ...
                cursor.getDouble(cursor.getColumnIndexOrThrow(COLUMN_VIDEOJUEGO_LATITUDE)),
                cursor.getDouble(cursor.getColumnIndexOrThrow(COLUMN_VIDEOJUEGO_LONGITUDE))
            )
